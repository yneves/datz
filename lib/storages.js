...
var Storage = classes.createClass({
  ...
  start: function(then,fail) {
    ...
    var addToQueue = function(type,model) {
      this.queue({
        type: type,
        model: model,
    ...
    var handleChange = function(event) {
      if (event.collection) {
        if (event.collection === this._collection) {
          event.added.forEach(addToQueue.bind(this,"insert"));
          event.removed.forEach(addToQueue.bind(this,"delete"));
        }
      } else if (event.model) {
        if (event.model._parent === this._collection) {
          addToQueue.call(this,"update",event.model);
        }
      }
    }.bind(this);
